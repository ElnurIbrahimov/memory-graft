...
def train_joint(
    ...
    # ALL params trainable, same learning rate
    for param in surgical_model.memory_block.parameters():
        param.requires_grad = True

    total_params = sum(
        p.numel() for p in surgical_model.memory_block.parameters()
    )
    print(f"\nJoint training: {total_params:,} params, all at lr={lr}")
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, len(indices), batch_size):
            ...
            for idx in batch_indices:
                ...
                key, value, h_orig = encode_fact_with_grad(
                ...
                # Reconstruction loss — direct gradient to write pathway
...
def main():
    ...
    parser.add_argument("--checkpoint_dir", default="checkpoints/qwen7b_joint_v1")
    ...
    # Load model — fresh memory block, no checkpoint
    ...
    print(f"\nMemory dim: {args.memory_dim} (bottleneck ratio: {sm.base_model.config.hidden_size / args.memory_dim:.1f}x)")
